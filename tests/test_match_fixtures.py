...
class MatchFixtureTests(unittest.TestCase):
    ...
    def test_fixture_scores_and_evidence_are_preserved(self):
        for name, data in self.fixtures.items():
            candidate = MatchCandidate(
                data["candidate_id"], data["score"], data["evidence"]
            )
            status = (
                ReviewStatus.CONFIRMED_MATCH
                if name in {"exact_match", "strong_partial_match"}
                else ReviewStatus.FALSE_POSITIVE
            )
    ...
    def test_fixtures_cover_distinct_detection_cases(self):
        self.assertEqual(set(self.fixtures), {
            "exact_match",
            "strong_partial_match",
            "lyrics_only",
            "metadata_only",
            "false_positive",
        })
